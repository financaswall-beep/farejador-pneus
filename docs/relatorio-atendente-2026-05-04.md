...
**Sprint 6.9 (action handlers + nota Chatwoot)** — `add_to_cart`, `remove_from_cart`, `update_cart_item`, `clear_cart`, `update_draft`, `request_confirmation` e `escalate` têm efeito em `agent.*`. `postEscalateNote` implementado: quando `escalate` é emitido, o worker posta nota interna (`private: true`) no Chatwoot via `ChatwootApiClient.createNote()`, **fora da transação**, após COMMIT — falha de API não reverte estado DB. Rótulo por motivo (`customer_requested`, `validator_blocked`, `confidence_low`, `pending_expired`, `ready_to_close`, `other`). No-op silencioso se variáveis de ambiente Chatwoot ausentes. 5 novos testes unitários. Suite: 313 testes passando. Deploy em produção: commit `e35ca31`, 2026-05-05.
