...
## Estado / o que falta
- **PUSHADO `7022462`, aguarda Deploy.** Pós-deploy conferir `?v=20260703-logistica` de fora. **Flags pra ligar no Coolify quando o dono quiser operar:** `MATRIZ_LOGISTICS=true` (a aba) e `MATRIZ_RECEIPT_AI=true` (a leitura por IA — precisa da `OPENAI_API_KEY` que já vive lá).
