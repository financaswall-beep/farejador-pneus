-- 0041_partner_summary_reads_partner_orders.sql
-- Corrige resumo/financeiro do Portal Parceiro.
--
-- Contexto:
--   A partir da migration 0040, vendas do parceiro sao gravadas em
--   commerce.partner_orders (silo proprio do parceiro), nao em commerce.orders.
--   A view network.partner_unit_summary ainda somava commerce.orders, por isso
--   as vendas apareciam na lista mas nao entravam no Resumo/Financeiro.
...
COMMENT ON VIEW network.partner_unit_summary IS
  'Resumo seguro do portal parceiro. Soma vendas de commerce.partner_orders, compras/despesas locais e estoque local. Nada toca bot/shadow/Chatwoot.';
