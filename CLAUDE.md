...
## 7. Estado atual (LIVE em prod — main `9044b2a`, conferido de fora 07-02 via `?v=20260702-fiado`)
- **Deploy 07-02 (main `9044b2a`) carregou:** FIADO do atacado LIVE (§11); trava de oversell do ATACADO (avisar+confirmar) live; oversell do VAREJO do bot dormente (`WHOLESALE_MATRIZ_OVERSELL_GUARD` off); **frete de entrega pelo PINO** (`DELIVERY_FREIGHT_FROM_PIN` LIGADA 07-02, palavra do dono — furo #696: bot media a retirada pelo pino mas exigia endereço escrito só pra COTAR a entrega; ⚠️ falta validar ao vivo repetindo o roteiro do #696). CANCELAR venda de atacado (`ed82fa5`), FATIA 2 lucro do varejo (`91a3694`) e COMISSÃO como LANÇAMENTO (0118, pushada 07-02 após banca 4×SHIP): aguardam Deploy (conferir `?v=20260703-comissao3` de fora; ligar `WHOLESALE_MATRIZ_RETAIL_COST` + `NETWORK_COMMISSION_LEDGER` no Coolify) (§11). Handoffs: `docs/SESSAO_2026-07-02_FRETE_PINO_FIADO_E_CANCELAMENTO_ATACADO_HANDOFF.md` + `..._2026-07-02b_FATIA2_...` + `..._2026-07-02c_COMISSAO_LANCAMENTO_HANDOFF.md` + `..._2026-07-02d_RETOMADA_ALARME_BANCA_E_PUSH_HANDOFF.md` (banca 4×SHIP + push `1eae2aa`).
...
- **LIVE em prod (main `9044b2a`, Deploy conferido 07-02):** **Fases 1-4** (venda pro borracheiro + ranking de recompra · estoque do galpão por MEDIDA + busca · custo+lucro · custo médio ponderado · medida em NÚMEROS larg/alt/aro `0113` validada contra o catálogo via `tireSizeKey`), **BAIXA do atacado** (`WHOLESALE_STOCK_DECREMENT` ON — confirmada em prod com venda real 5un: 35→30), **BAIXA do galpão no VAREJO da matriz** (bot+balcão, `applyMatrizGalpaoDecrement`, `WHOLESALE_MATRIZ_DECREMENT` ON), **fornecedores/compras** (`0114`), **trava de oversell do atacado** (avisar+confirmar + lock de linha) e o **FIADO** (`0115` + flag `WHOLESALE_FINANCE` ON — bloco "Financeiro do galpão": a receber/a pagar/vencidos + botão Recebi/Paguei + seletor à vista×fiado nos forms de venda e compra; ⚠️ falta validar ao vivo com venda fiada real).
- **PUSHADO aguardando Deploy (`ed82fa5`):** **CANCELAR venda de atacado** (`0116` APLICADA — trilha cancelled_at/by/reason; DEVOLVE o estoque como espelho exato da baixa; cancelada some SOZINHA do fiado/ranking/resumo; tabela "Últimas vendas" com badge + botão Cancelar + aviso forte se venda paga; SEM flag, aditivo; provado 13/13; ⚠️ assimetria do clamp em oversell documentada no código — correção real = livro-razão/Camada 2, adiada).
- **PUSHADO aguardando Deploy (`91a3694`, fatia 2):** **LUCRO REAL DO VAREJO da matriz** (`0117` APLICADA — bot+balcão+manual da 'main' CONGELAM o custo médio do galpão em `order_items.matriz_unit_cost`; flag `WHOLESALE_MATRIZ_RETAIL_COST` default OFF dormente — LIGAR no Coolify pós-Deploy; card "Financeiro do varejo" Faturamento/Custo/Lucro do SERVIDOR + banner de item sem custo; recorte **Tudo × Esse mês** nos cards do varejo E do atacado, fuso São Paulo; endpoint `/admin/api/varejo/resumo`; provado 15/15 + 519 unit + preview).
- **PUSHADO 07-02 aguardando Deploy (obra 4; banca de 4 especialistas = 4×SHIP):** **COMISSÃO como LANÇAMENTO** (`0118` APLICADA — `network.commission_entries`, ZERO grant pro parceiro; regras do dono 07-02: nasce quando a venda 2W REALIZA (régua 0077/0090) · cancelou → estorna sozinho (pós-paga = "acerto por fora") · % da FICHA congelado no lançamento · FRETE FORA da base (comissão só sobre os pneus); VARREDURA idempotente no GET da tela (sem gancho no fluxo do parceiro/bot); bloco "Comissões a receber" na página Rede (UM Recebi por LOJA com total ACUMULADO; extrato lançamento-a-lançamento ESCONDIDO atrás de "ver extrato"; **alarme COBRAR**: dono define R$ X no localStorage → loja com aberto ≥ X pisca badge + botão "Cobrar no WhatsApp" via wa.me com msg pronta — pedido do dono 07-02) + **editor do modelo comercial** na página da unidade (pendência de 06-01 QUITADA, com audit.events); flag `NETWORK_COMMISSION_LEDGER` default OFF; provado 18/18 (incl. frete fora) + 519 unit + preview (alarme + Recebi validados no 4213 pelo dono); com a flag on, o card antigo "A receber da rede" (Resumo+Rede+drill-down) LÊ O LIVRO em vez da estimativa % × vendas (furo apontado pelo dono: quitava e o card não descontava); `?v=20260703-comissao3`; mensalidade fica pra próxima fatia — decisão do dono).
...
- Migrations **0110–0117** (0114 fornecedores; 0115 fiado; 0116 trilha de cancelamento; 0117 custo congelado do varejo — todas APLICADAS). Detalhe: `docs/SESSAO_2026-07-02_FRETE_PINO_FIADO_E_CANCELAMENTO_ATACADO_HANDOFF.md` (fiado + cancelamento + varredura do financeiro) + `..._2026-06-27_GALPAO_*` + `..._2026-06-22_ATACADO_*`.
...
- **Roadmap (ordem de dor):** **mensalidade como lançamento** (⚠️ banca 07-02: precisa sair ANTES de ativar modelo monthly/hybrid pra parceiro real — hoje ninguém usa; mensalidade NÃO nasce de venda, é por competência/mês — desenho próprio, não pendurar no sweep de vendas); **0119 aditiva** (índice parcial 2w em partner_orders — sweep faz Seq Scan a cada GET da Rede — + trava física de grant no livro, espelho da 0110); **consolidado das 3 pernas DESBLOQUEADO** (varejo custo ✓ + atacado ✓ + comissão ✓ — tela única "faturei/lucrei/a receber" = só leitura das 3 fontes); ligar+validar `WHOLESALE_MATRIZ_OVERSELL_GUARD`; validar ENTREGA pela matriz ao vivo (frete 9,90/13/19); faxina de chaves (combinada 07-02). Adiados DE PROPÓSITO: PEPS por fornecedor, Camada 2 (reserva + livro-razão + quitação PARCIAL de comissão — "R$200 por conta" hoje não tem meio-Recebi).
