...
- **PRONTO LOCAL, NÃO pushado (fatia 2, 07-02):** **LUCRO REAL DO VAREJO da matriz** (`0117` APLICADA — bot+balcão+manual da 'main' CONGELAM o custo médio do galpão em `order_items.matriz_unit_cost`; flag `WHOLESALE_MATRIZ_RETAIL_COST` default OFF dormente; card "Financeiro do varejo" Faturamento/Custo/Lucro do SERVIDOR + banner de item sem custo — honestidade > chute; recorte **Tudo × Esse mês** nos cards do varejo E do atacado, fuso São Paulo; endpoint `/admin/api/varejo/resumo`; sem grant nenhum de app em orders/order_items = custo não vaza pro parceiro; provado 15/15 integração + 519 unit + regressão completa + preview 4212; `?v=20260702-varejo-lucro`).
...
- **Roadmap (ordem de dor):** comissão virar lançamento (a receber por parceiro — regras são do DONO: quando é devida / cancelamento / percentual, editor do modelo comercial pendente de 06-01) → aí sim o **consolidado das 3 pernas**; ligar+validar `WHOLESALE_MATRIZ_OVERSELL_GUARD`; validar ENTREGA pela matriz ao vivo (frete 9,90/13/19). Adiados DE PROPÓSITO: PEPS por fornecedor ("quem pesa na média"/"qual encalha" — operar o médio primeiro) e Camada 2 (reserva + livro-razão da matriz).
