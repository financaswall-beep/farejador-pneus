...
- **Deploy 07-02 (main `9044b2a`) carregou:** FIADO do atacado LIVE (§11); trava de oversell do ATACADO (avisar+confirmar) live; oversell do VAREJO do bot dormente (`WHOLESALE_MATRIZ_OVERSELL_GUARD` off); **frete de entrega pelo PINO** dormente (`DELIVERY_FREIGHT_FROM_PIN` off — furo #696: bot media a retirada pelo pino mas exigia endereço escrito só pra COTAR a entrega; ligar a flag e validar repetindo o roteiro do #696). CANCELAR venda de atacado: pronto LOCAL, aguarda push (§11). Handoff: `docs/SESSAO_2026-07-02_FRETE_PINO_FIADO_E_CANCELAMENTO_ATACADO_HANDOFF.md`.
...
- **PRONTO LOCAL, NÃO pushado (aguarda ordem do dono):** **CANCELAR venda de atacado** (`0116` APLICADA — trilha cancelled_at/by/reason; DEVOLVE o estoque como espelho exato da baixa; cancelada some SOZINHA do fiado/ranking/resumo; tabela "Últimas vendas" com badge + botão Cancelar + aviso forte se venda paga; SEM flag, aditivo; provado 13/13 + regressão completa; ⚠️ assimetria do clamp em oversell documentada no código — correção real = livro-razão/Camada 2, adiada).
...
- Migrations **0110–0116** (0114 fornecedores; 0115 fiado; 0116 trilha de cancelamento — todas APLICADAS). Detalhe: `docs/SESSAO_2026-07-02_FRETE_PINO_FIADO_E_CANCELAMENTO_ATACADO_HANDOFF.md` (fiado + cancelamento + varredura do financeiro) + `..._2026-06-27_GALPAO_*` + `..._2026-06-22_ATACADO_*`.
...
- **Roadmap (ordem de dor):** **fatia 2 do financeiro** = custo congelado no VAREJO da matriz (lucro do varejo hoje é cego) + comissão virar lançamento → aí sim o consolidado das 3 pernas; resumo do atacado com recorte por período (hoje é all-time); ligar+validar `WHOLESALE_MATRIZ_OVERSELL_GUARD`; validar ENTREGA pela matriz ao vivo (frete 9,90/13/19). Adiados DE PROPÓSITO: PEPS por fornecedor ("quem pesa na média"/"qual encalha" — operar o médio primeiro) e Camada 2 (reserva + livro-razão da matriz).
