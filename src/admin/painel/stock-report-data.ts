...
export async function readStockReportSnapshot(db: Pool, environment: string, filter: StockReportFilter): Promise<StockReportSnapshot> {
  ...
  try {
    ...
    const stocks=await client.query<StockReportVariant>(`WITH pending AS (
      SELECT i.measure,COALESCE(i.brand,'Sem marca') brand,i.tire_condition,i.vehicle_type,
      ...
      WHERE i.environment=$1 AND p.status='pending' GROUP BY i.measure,COALESCE(i.brand,'Sem marca'),i.tire_condition,i.vehicle_type
    ), identities AS (
      SELECT measure,brand,tire_condition,commerce.stock_vehicle_type(environment,measure,brand,tire_condition) vehicle_type FROM commerce.wholesale_stock WHERE environment=$1
      UNION SELECT measure,brand,tire_condition,vehicle_type FROM pending
      UNION SELECT rp.measure,'Sem marca',rp.tire_condition,rp.vehicle_type FROM commerce.wholesale_replenishment_policies rp
        WHERE rp.environment=$1 AND NOT EXISTS (SELECT 1 FROM commerce.wholesale_stock s
          WHERE s.environment=rp.environment AND s.measure=rp.measure AND s.tire_condition=rp.tire_condition AND commerce.stock_vehicle_type(s.environment,s.measure,s.brand,s.tire_condition) IS NOT DISTINCT FROM rp.vehicle_type)
          AND NOT EXISTS(SELECT 1 FROM pending p WHERE p.measure=rp.measure AND p.tire_condition=rp.tire_condition AND p.vehicle_type IS NOT DISTINCT FROM rp.vehicle_type)
    ) SELECT k.measure,k.brand,k.tire_condition AS "condition",k.vehicle_type,COALESCE(s.quantity_on_hand,0)::int physical,
    ...
    FROM identities k LEFT JOIN commerce.wholesale_stock s ON s.environment=$1
      AND s.measure=k.measure AND s.brand=k.brand AND s.tire_condition=k.tire_condition AND commerce.stock_vehicle_type(s.environment,s.measure,s.brand,s.tire_condition) IS NOT DISTINCT FROM k.vehicle_type
    LEFT JOIN pending p ON p.measure=k.measure AND p.brand=k.brand AND p.tire_condition=k.tire_condition AND p.vehicle_type IS NOT DISTINCT FROM k.vehicle_type
    LEFT JOIN commerce.wholesale_replenishment_policies rp ON rp.environment=$1
      AND rp.measure=k.measure AND rp.tire_condition=k.tire_condition AND rp.vehicle_type IS NOT DISTINCT FROM k.vehicle_type
    ...
    const events=await client.query<Omit<StockReportMovement,'at'> & {at:Date}>(`SELECT id,measure,brand,tire_condition AS "condition",vehicle_type,
