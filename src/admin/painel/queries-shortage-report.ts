...
const countSearches=(rows:ShortageTrace[])=>new Set(rows.map(t=>t.search_key)).size;
export function buildShortageReport(data:ShortageSnapshot,f:ShortageReportFilter){
  ...
  for(const s of list)s.consultations=countSearches(traces.filter(t=>t.stores.some(x=>x.id===s.id&&x.available===false)));
  ...
  const allMeasures=[...grouped].map(([key,items])=>{
    ...
    return{key,measure:items[0]!.measure,shortages:items.length,
  ...
  const consultations=selectedRows.map(({conversation_id:_,search_key:__,...t})=>({...t,measure_key:tireSizeKey(t.measure),
  ...
  return{filters:{...f,store:selected?.id||f.store},as_of:data.as_of,tracking_since:data.tracking_since,legacy_records:data.legacy_records,
    summary:{consultations:countSearches(traces),shortages:list.reduce((n,s)=>n+s.shortages,0),stores:list.length,measures:new Set(traces.map(t=>tireSizeKey(t.measure))).size},
