...
export async function getStockCosts(db: Pool = defaultPool) {
  const result = await db.query(`WITH variants AS (
    ...
    SELECT measure||':'||tire_condition AS key,measure,tire_condition,
    ...
    FROM variants GROUP BY measure,tire_condition
  ), lots AS (
    SELECT l.id,'LT-'||lpad(l.lot_number::text,6,'0') lot_code,l.description,
      l.quantity_on_hand,l.quantity_reserved,l.quantity_on_hand-l.quantity_reserved quantity_available,
    ...
    'as_of',now()) payload FROM catalog_summary c CROSS JOIN lot_summary l`, [env.FAREJADOR_ENV]);
