...
import {
  ...
  listMatrizCollaborators,
  createMatrizCollaborator,
  updateMatrizCollaboratorJob,
  revokeMatrizCollaborator,
  reactivateMatrizCollaborator,
  resetMatrizCollaboratorPassword,
...
export async function registerPainelRoute(fastify: FastifyInstance): Promise<void> {
  ...
  const logisticaRecolocarSchema = z.object({
    order_id: z.string().uuid(),
  });
