...
export function shortageReportCsv(r:ReturnType<typeof buildShortageReport>){
  ...
  if(r.filters.view==='consultations'){
    rows.push(['Histórico de buscas','Inclui repetições; a quantidade de linhas não representa o total de faltas.']);
    rows.push(['Data e hora','Medida','Município','Filtros da busca','Lojas consultadas','Resultado','Referência atual (não somar as consultas)','Buscas da mesma conversa e medida nesta loja']);
    for(const t of r.consultations)rows.push([t.occurred_at,t.measure,t.municipality,Object.entries(t.filters).map(([k,v])=>k+': '+v).join(' · '),
      t.stores.map(s=>s.name+': '+(s.available?'tinha':'não tinha')).join(' | '),t.available_elsewhere?'Havia em outra loja consultada':'Sem disponibilidade nas lojas consultadas',money(t.reference?.amount??null),t.searches]);
