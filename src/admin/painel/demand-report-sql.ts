...
export const demandEventsSql=`WITH bounds AS (
  ...
  SELECT f.conversation_id,COALESCE(f.observed_at,f.created_at),
    ...
    CASE WHEN f.fact_key='medida_consultada' AND jsonb_typeof(f.fact_value)='string'
      THEN NULLIF(btrim(f.fact_value#>>'{}'),'') ELSE NULL END,f.vehicle_type
  ...
  UNION ALL
  SELECT s.conversation_id,s.occurred_at,k.kind,s.measure,s.vehicle_type
  ...
  UNION ALL
  SELECT o.source_conversation_id,o.created_at,'order',NULL,oi.vehicle_type
  FROM commerce.orders o LEFT JOIN LATERAL(SELECT DISTINCT vehicle_type FROM commerce.order_items i WHERE i.environment=o.environment AND i.order_id=o.id) oi ON true
  LEFT JOIN commerce.partner_orders po ON po.id=o.partner_order_id AND po.environment=o.environment,bounds b
  ...
  UNION ALL
  SELECT o.source_conversation_id,CASE WHEN o.partner_order_id IS NULL THEN o.delivered_at ELSE po.delivered_at END,'delivery',NULL,oi.vehicle_type
  FROM commerce.orders o LEFT JOIN LATERAL(SELECT DISTINCT vehicle_type FROM commerce.order_items i WHERE i.environment=o.environment AND i.order_id=o.id) oi ON true
  LEFT JOIN commerce.partner_orders po ON po.id=o.partner_order_id AND po.environment=o.environment,bounds b
  WHERE o.environment=$1 AND o.source_conversation_id IS NOT NULL AND o.status<>'cancelled'
  ...
  SELECT e.*,COALESCE(e.vehicle_type,CASE WHEN e.measure IS NOT NULL THEN
    (SELECT CASE WHEN count(DISTINCT known.vehicle_type)=1 THEN min(known.vehicle_type) END FROM events known
      WHERE known.conversation_id=e.conversation_id AND commerce.catalog_measure_identity(known.measure)=commerce.catalog_measure_identity(e.measure)) END) resolved_type
  FROM events e
), grouped AS (
  SELECT e.conversation_id,e.kind,CASE WHEN e.kind='measure' THEN e.measure END AS measure,to_char(min(e.at) AT TIME ZONE 'America/Sao_Paulo','YYYY-MM-DD') AS day
  FROM classified e JOIN core.conversations c ON c.id=e.conversation_id AND c.environment=$1 AND c.deleted_at IS NULL
