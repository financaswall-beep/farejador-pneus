  ...
  try{
    ...
    const meta=(await c.query<{as_of:Date;tracking_since:Date|null;legacy_records:number}>(`SELECT now() AS as_of,
      ...
      (SELECT count(*)::int FROM analytics.conversation_facts f WHERE f.environment=$1 AND f.fact_key='faltou_estoque'
        ...
        AND ($4='all' OR ($4='unknown' AND f.vehicle_type IS NULL) OR f.vehicle_type=$4)
        ...
        AND NOT EXISTS(SELECT 1 FROM ops.bot_stock_searches s WHERE s.environment=f.environment
          ...
          AND s.measure=upper(btrim(f.fact_value->>'medida')))) AS legacy_records`,[...params,f.vehicle_type || 'all'])).rows[0]!;
    const traces=(await c.query<Omit<ShortageTrace,'occurred_at'>&{occurred_at:Date}>(`SELECT id,conversation_id,search_key,occurred_at,measure,municipality,filters,stores,vehicle_type
      FROM ops.bot_stock_searches WHERE environment=$1 AND ${dateRange} AND ($4='all' OR ($4='unknown' AND vehicle_type IS NULL) OR vehicle_type=$4)
      AND EXISTS(SELECT 1 FROM jsonb_array_elements(stores) st WHERE st->>'available'='false')
      ORDER BY occurred_at DESC,id LIMIT 20001`,[...params,f.vehicle_type || 'all'])).rows;
    ...
    const products=traces.length?(await c.query<ShortageProduct>(`SELECT p.id,ts.tire_size AS measure,p.brand,p.tire_condition AS condition,ts.position,
      ...
      WHERE p.environment=$1 AND p.deleted_at IS NULL AND p.product_type='tire' AND ($2='all' OR ($2='unknown' AND ts.vehicle_type IS NULL) OR ts.vehicle_type=$2) ORDER BY p.id LIMIT 20001`,[environment,f.vehicle_type || 'all'])).rows:[];
    const stock=traces.length?(await c.query<ShortageStock>(`SELECT 'matriz'::text AS store_id,measure,
      ...
      FROM commerce.wholesale_stock WHERE environment=$1 AND ($3='all' OR ($3='unknown' AND commerce.stock_vehicle_type(environment,measure,brand,tire_condition) IS NULL) OR commerce.stock_vehicle_type(environment,measure,brand,tire_condition)=$3)
      ...
      FROM commerce.partner_stock_levels WHERE environment=$1 AND ($3='all' OR ($3='unknown' AND commerce.catalog_vehicle_type(environment,tire_size,brand,tire_condition) IS NULL) OR commerce.catalog_vehicle_type(environment,tire_size,brand,tire_condition)=$3) AND unit_id::text=ANY($2::text[]) AND deleted_at IS NULL
      LIMIT 50001`,[environment,ids,f.vehicle_type || 'all'])).rows:[];
