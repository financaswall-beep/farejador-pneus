...
export async function listGalpaoMovements(
  opts: {
    vehicle_type?: TireVehicleFilter;
    measure?: string | null;
    brand?: string | null;
  ...
  const result = await dbPool.query<GalpaoMovementRow>(
    `SELECT vehicle_type,measure,brand,tire_condition,op,qty_before,qty_after,qty_delta,
      ...
      AND ($6='all' OR COALESCE(vehicle_type,'unknown')=$6)
      ORDER BY created_at DESC,id DESC
      LIMIT $5`,
    [environment, measure, brand, tireCondition, limit, opts.vehicle_type || 'all'],
  );
  return result.rows;
}
