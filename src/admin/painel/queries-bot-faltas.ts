...
export interface ShortagePeriod { from: string; to: string; vehicle_type?: string }
...
const base = `WITH searches AS (
  ...
  WHERE environment=$1 AND occurred_at >= ($2::date::timestamp AT TIME ZONE 'America/Sao_Paulo')
    ...
    AND ($4='all' OR COALESCE(vehicle_type,'unknown')=$4)
), missing AS (
...
function params(period: ShortagePeriod, environment: string) { return [environment, period.from, period.to, period.vehicle_type || 'all']; }
...
export async function getBotShortageConsultations(filter: ShortageFilter, environment = env.FAREJADOR_ENV, db: Pool = pool) {
  ...
  const result = await db.query(`${base}, selected AS (
    SELECT DISTINCT id FROM missing WHERE ($5='' OR store_id=$5) AND measure_key=commerce.wholesale_measure_key($6)
    ...
    ORDER BY s.occurred_at DESC,s.id LIMIT 20 OFFSET $7`, values);
  const stock = await db.query(`WITH quantities AS (
      ...
      AND ($3='all' OR COALESCE(commerce.stock_vehicle_type(environment,measure,brand,tire_condition),'unknown')=$3)
      HAVING count(*)>0
      UNION ALL
      ...
      AND ($3='all' OR COALESCE(commerce.catalog_vehicle_type(sl.environment,sl.tire_size,sl.brand,sl.tire_condition),'unknown')=$3)
      GROUP BY sl.unit_id
    ) SELECT * FROM quantities`, [environment, filter.measure, filter.vehicle_type || 'all']);
...
export async function exportBotShortages(filter: ShortageFilter, environment = env.FAREJADOR_ENV, db: Pool = pool) {
  return (await db.query(`${base} SELECT id,occurred_at,measure,municipality,store_name,filters,searches,
    ...
    FROM missing_once WHERE ($5='' OR store_id=$5)
      AND ($6='' OR commerce.wholesale_measure_key(measure) LIKE '%'||commerce.wholesale_measure_key($6)||'%')
