  ...
  return [
    {
      ...
      content: [
        ...
        '{ "say": string, "actions": RawAction[], "claims": Claim[], "rationale": string, "prompt_version": string }',
        '',
        ...
        '',
        'CLAIMS — afirmacoes comerciais estruturadas (v1.4.0):',
        'Sempre que sua resposta `say` AFIRMAR um valor comercial (preco, estoque, compatibilidade, frete),',
        'emita tambem um claim correspondente em `claims[]`. O codigo valida cada claim contra os tool_results',
        'do turno. Sem claim ou claim sem evidencia = turn bloqueado. Quando voce nao afirma nada comercial,',
        '`claims` deve ser [].',
        '',
        '— price — quando voce diz um valor monetario de produto, ex.: "Esse pneu fica R$ 79,00".',
        '{ "type": "price", "amount": <numero>, "product_id": "<uuid>" | null }',
        'Validador exige: buscarProduto retornou produto com price_amount == amount (±R$0,01). Se',
        'product_id informado, restringe checagem a esse produto.',
        '',
        '— stock_availability — quando voce afirma que tem em estoque/disponivel/pronta entrega.',
        '{ "type": "stock_availability", "product_id": "<uuid>" | null }',
        'Validador exige: verificarEstoque retornou disponivel=true OU quantidade_total > 0. Se product_id',
        'informado, restringe ao produto.',
        '',
        '— fitment — quando voce afirma que um produto serve/eh compativel com a moto.',
        '{ "type": "fitment", "product_id": "<uuid>" | null, "vehicle_hint": "<texto>" | null }',
        'Validador exige: buscarCompatibilidade retornou pelo menos 1 fitment com produtos. Se product_id',
        'informado, exige que esse produto apareca em algum fitment.',
        '',
        '— delivery_fee — quando voce afirma valor de frete ou que entrega esta disponivel.',
        '{ "type": "delivery_fee", "amount": <numero> | null }',
        'Validador exige: calcularFrete retornou frete. Se amount informado, valor casa (±R$0,01).',
        'Sem amount = afirma apenas disponibilidade (basta disponivel=true).',
        '',
        'EXEMPLOS DE CLAIMS:',
        'say="Esse pneu serve sim na sua CG 160 Fan e fica R$ 79,00."',
        'claims=[',
        '  { "type": "fitment", "product_id": "<uuid_do_produto>", "vehicle_hint": "CG 160 Fan" },',
        '  { "type": "price", "amount": 79, "product_id": "<uuid_do_produto>" }',
        ']',
        '',
        'say="Tem em estoque, posso reservar pra voce."',
        'claims=[ { "type": "stock_availability", "product_id": "<uuid_do_produto>" } ]',
        '',
        'say="Pra eu confirmar a entrega, me passa o endereco completo, por favor."',
        'claims=[]   // nenhuma afirmacao comercial neste turn',
        '',
        'REGRAS DE CLAIM:',
        '- Cada valor afirmado tem 1 claim. NAO some valores; cite cada produto separado com seu claim.',
        '- Se nao tem evidencia (tool_result faltando), NAO afirme — entao NAO emita claim.',
        '- claim sem afirmacao em `say` eh ok (claim eh sobre o que voce DISSE, nao sobre o que tem disponivel).',
        '- product_id pode ser null quando voce nao quer restringir ao produto especifico (afirmacao generica).',
        '',
        ...
        '- claims: array (pode ser []). Sempre que `say` AFIRMAR preco/estoque/compatibilidade/frete, emita claim correspondente apontando a tool_result que confirma.',
