...
async function teamRows(
  ...
  const [team, settlements] = await Promise.all([
    ...
    db.query<SettlementRow>(
      `SELECT item.token_id,item.payable_id,COALESCE(item.total_due,0)::text payable_amount,
              payable.status payable_status
         FROM finance.partner_payroll_items item
         JOIN finance.partner_payroll_periods payroll
           ON payroll.environment=item.environment AND payroll.id=item.payroll_period_id
         LEFT JOIN finance.partner_payables payable
           ON payable.environment=item.environment AND payable.id=item.payable_id
        WHERE item.environment=$1 AND payroll.unit_id=$2
          AND payroll.competence_month >= date_trunc('month',$3::date)::date
          AND payroll.competence_month < date_trunc('month',$4::date)::date + interval '1 month'
        ORDER BY payroll.competence_month DESC`,
      [ctx.environment, ctx.unitId, bounds.start, bounds.end],
    ),
  ]);
...
function collaboratorOf(row: TeamRow, settlements: SettlementRow[]): OperationCommissionCollaborator {
  ...
  const canPay = Number(row.unsettled_count || 0) === 0 && open.length === 1;
  return {
    id: row.token_id,
...
export async function getPartnerOperationCommissionDetail(
  ...
  const result = await db.query<{
    ...
    [ctx.environment, ctx.unitId, collaboratorId, bounds.start, bounds.end],
  );
  ...
  const allowed = await db.query(
    `SELECT 1
       ...
       JOIN finance.partner_payables payable
         ON payable.environment=item.environment AND payable.id=item.payable_id
      WHERE item.environment=$1 AND period.unit_id=$2 AND item.token_id=$3
        AND item.payable_id=$4 AND payable.status='open' AND payable.deleted_at IS NULL`,
