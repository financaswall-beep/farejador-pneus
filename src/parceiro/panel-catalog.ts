...
export async function getPartnerPanelCatalog(
  ...
  return withPartnerContext(ctx.partnerUnitId, async (client) => {
    const filters = [ctx.environment, like, brand, type];
    ...
    const brandsResult = await client.query<{ brand: string }>(
      `SELECT DISTINCT btrim(brand) brand
      ...
      `SELECT p.id product_id,p.product_code,p.product_name,p.product_type,
         ...
         LEFT JOIN LATERAL (
           SELECT count(*)::int stock_rows,
            ...
            WHERE psl.environment=p.environment AND psl.unit_id=$5
        ...
        LIMIT $6 OFFSET $7`,
      [...filters, ctx.unitId, limit, (page - 1) * limit],
    );
