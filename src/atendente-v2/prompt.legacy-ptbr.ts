/**
 * PROMPT LEGADO EM PT-BR (snapshot 2026-05-26)
 *
 * Este arquivo guarda o SYSTEM_PROMPT que estava em produção ate 2026-05-26
 * (commit b7fd8f1 e anteriores). Foi substituido pela versao hibrida em
 * ingles+exemplos pt-br no commit que segue, com objetivo de economizar
 * ~37-40% em tokens de prompt.
 *
 * Por que ficou aqui:
 *   - Fallback rapido: se a versao em ingles vazar idioma ou regredir
 *     comportamento, basta importar LEGACY_SYSTEM_PROMPT_PTBR em
 *     prompt.ts e refazer o deploy.
 *   - Referencia historica: 8 commits desta sessao corrigiram bugs
 *     observados em producao. Cada regra desse texto tem origem
 *     concreta (PED-0005, PED-0006, etc).
 *
 * Como reverter pro pt-br:
 *   1. Em src/atendente-v2/prompt.ts, trocar:
 *      `export const SYSTEM_PROMPT = \`...ingles...\`;`
 *      por:
 *      `export { LEGACY_SYSTEM_PROMPT_PTBR as SYSTEM_PROMPT } from './prompt.legacy-ptbr.js';`
 *   2. Commit + push pneus
 *   3. Aguardar redeploy no Coolify (~2min)
 *   4. Cache da OpenAI vai rebuildar nas primeiras 5-10 convs (~R$ 0,50)
 *
 * Tokens nominais: ~2.852 (pt-br ratio)
 *
 * Veja tambem: docs/AGENT_V2_PROMPT_EXPERIMENTO_INGLES.md
 */
export const LEGACY_SYSTEM_PROMPT_PTBR = `Você é a atendente de uma loja de pneus de moto. Atende pelo WhatsApp.
