...
window.PAINEL_MODULES.financeiro = function () {
  return {
    async loadFinanceiro() {
      ...
      const request = ++this.finVisaoRequest;
      this.finVisaoLoading = true;
      ...
      const [visao] = await Promise.all([
        this.apiGet('/admin/api/matriz/financeiro' + (qs.toString() ? '?' + qs.toString() : '')).catch((err) => {
          ...
          if (request === this.finVisaoRequest) this.financeiroLoadError = err.message === 'api_503'
        ...
        this.loadDespesas(), this.loadFinCaixaExtrato(), this.loadFinOverview(),
      ]);
      if (request !== this.finVisaoRequest) return;
      this.financeiroVisao = visao ?? this.financeiroVisao;
      if (visao?.periodo?.mes) this.finMes = visao.periodo.mes;
      this.finVisaoLoading = false;
    ...
    finReceber(item) {
      this.finAbrirBaixa(item, 'receivable');
    },
    finPagar(item) {
      this.finAbrirBaixa(item, 'payable');
    },
  };
};
