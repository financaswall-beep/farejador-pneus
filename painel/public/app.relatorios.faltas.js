...
window.PAINEL_MODULES.relatoriosFaltas=function(){
  ...
  return{
    rfal: {vehicle_type:'all',tab:'overview',mode:'month',month:today().slice(0,7),from:'',to:'',store:'',measure:'',search:'',storeSearch:'',allStores:false,
    ...
    rfalQuery(filters=null,view=this.rfal.tab){const f=filters||this.rfal;return new URLSearchParams({vehicle_type:f.vehicle_type || 'all',from:f.from,to:f.to,mode:f.mode,store:f.store,measure:f.measure,search:f.search,view}).toString();},
    ...
    rfalRestoreView(){try{const saved=JSON.parse(localStorage.getItem(this.rfalStorageKey())||'null');if(!saved?.filters)return;
      ...
      this.rfal.notice='Visão restaurada. Preços, estoque e faltas consultados novamente.';this.rfal.vehicle_type = ['motorcycle','car','mixed','unknown'].includes(saved.filters.vehicle_type) ? saved.filters.vehicle_type : 'all';void this.rfalLoad();}catch(_){this.rfal.notice='Não foi possível restaurar a visão. Selecione os filtros novamente.';}},
  };
};
