...
window.PAINEL_MODULES.relatoriosDemanda=function(){
  ...
  return{
    rdem: {vehicle_type:'all',tab:'overview',mode:'month',month:today().slice(0,7),from:'',to:'',compare:true,city:'',citySearch:'',measure:'',search:'',
    ...
    rdemQuery(filters=null){const f=filters||this.rdem;return new URLSearchParams({vehicle_type:f.vehicle_type || 'all',from:f.from,to:f.to,mode:f.mode,compare:String(f.compare),city:f.city,citySearch:f.citySearch,
    ...
    rdemReset(){Object.assign(this.rdem,{vehicle_type:'all',city:'',citySearch:'',measure:'',search:'',sort:'conversations',metric:'conversations'});void this.rdemLoad();},
    ...
    rdemRestoreView(){try{const f=JSON.parse(localStorage.getItem(this.rdemStorageKey())||'null');if(!f)return;
      ...
      this.rdem.notice='Visão restaurada. Os dados foram consultados novamente.';this.rdem.vehicle_type = ['motorcycle','car','mixed','unknown'].includes(f.vehicle_type) ? f.vehicle_type : 'all';void this.rdemLoad();}catch(_){this.rdem.notice='Não foi possível restaurar a visão. Selecione os filtros novamente.';}},
  };
};
