...
describe('Login — funcionário criado pelo dono', () => {
  it('loga com usuário+senha e a sessão valida com papel funcionario', async () => {
    ...
    const created = await q.createPartnerFuncionario(f.ctx, 'João Balcão', 'joao', 'senha-segura-123');
    expect(created.username).toBe('joao');

    const login = await q.authenticatePartnerLogin('test', f.slug, 'joao', 'senha-segura-123');
  ...
  it('é case-insensitive no usuário', async () => {
    const q = await importQueries();
    const f = await createPartnerFixture(db.pool);
    await q.createPartnerFuncionario(f.ctx, null, 'Maria', 'senha-segura-123');
    const login = await q.authenticatePartnerLogin('test', f.slug, 'maria', 'senha-segura-123');
    expect(login).not.toBeNull();
  });
});
...
describe('Login — credenciais inválidas', () => {
  it('senha errada → null', async () => {
    const q = await importQueries();
    const f = await createPartnerFixture(db.pool);
    await q.createPartnerFuncionario(f.ctx, null, 'pedro', 'senha-certa-123');
...
describe('Login — primeiro acesso do dono', () => {
  it('set-credentials por token cru define usuário+senha e já devolve sessão', async () => {
    ...
    const result = await q.setOwnPartnerCredentials(f.ctx, 'dono', 'senha-do-dono-123', true);
    ...
    const login = await q.authenticatePartnerLogin('test', f.slug, 'dono', 'senha-do-dono-123');
    expect(login).not.toBeNull();
    const ctx = await auth.authenticatePartnerSession(f.slug, login!.session_token);
...
describe('Login — revogar funcionário', () => {
  it('sessão para de validar assim que o login é revogado', async () => {
    ...
    const auth = await importAuth();
    const f = await createPartnerFixture(db.pool);
    const created = await q.createPartnerFuncionario(f.ctx, null, 'temp', 'senha-segura-123');
    const login = await q.authenticatePartnerLogin('test', f.slug, 'temp', 'senha-segura-123');
    ...
    // E não loga mais.
    expect(await q.authenticatePartnerLogin('test', f.slug, 'temp', 'senha-segura-123')).toBeNull();
  });

  it('preserva o histórico ao reativar e não revoga uma pessoa ainda ativa na Matriz', async () => {
    const q = await importQueries();
    const f = await createPartnerFixture(db.pool);
    const created = await q.createPartnerFuncionario(
      f.ctx, 'Vínculo duplo', 'vinculo.duplo', 'senha-segura-123',
    );
    const token = await db.pool.query<{ person_id: string }>(
      `SELECT person_id FROM network.partner_access_tokens WHERE id=$1`, [created.id],
    );
    await db.pool.query(
      `INSERT INTO network.matriz_collaborators
        (environment,person_id,display_name,job,job_title,work_area)
       VALUES ('test',$1,'Vínculo Duplo','colaborador','Auxiliar','other')`,
      [token.rows[0]!.person_id],
    );

    await q.revokePartnerFuncionario(f.ctx, created.id);
    const person = await db.pool.query<{ revoked_at: string | null }>(
      `SELECT revoked_at FROM network.partner_people WHERE id=$1`, [token.rows[0]!.person_id],
    );
    expect(person.rows[0]!.revoked_at).toBeNull();
    await q.reactivatePartnerFuncionario(f.ctx, created.id, db.pool);
    const periods = await db.pool.query<{ ended_at: string | null }>(
      `SELECT ended_at FROM network.partner_collaborator_employment_periods
        WHERE environment='test' AND token_id=$1 ORDER BY started_at`, [created.id],
    );
    expect(periods.rows).toHaveLength(2);
    expect(periods.rows[0]!.ended_at).not.toBeNull();
    expect(periods.rows[1]!.ended_at).toBeNull();
  });
});
...
describe('Login — sessão expirada', () => {
  it('não valida depois de expires_at', async () => {
    ...
    const f = await createPartnerFixture(db.pool);
    await q.createPartnerFuncionario(f.ctx, null, 'expira', 'senha-segura-123');
    const login = await q.authenticatePartnerLogin('test', f.slug, 'expira', 'senha-segura-123');
...
describe('Login — unicidade do usuário', () => {
  it('mesma unidade não aceita usuário duplicado', async () => {
    const q = await importQueries();
    const f = await createPartnerFixture(db.pool);
    await q.createPartnerFuncionario(f.ctx, null, 'dup', 'senha-segura-123');
    await expect(q.createPartnerFuncionario(f.ctx, null, 'dup', 'outra-segura-123'))
  ...
  it('a porta unica nao permite duas pessoas ativas com o mesmo usuario', async () => {
    ...
    await q.createPartnerFuncionario(a.ctx, null, username, 'senha-segura-123');
    await expect(q.createPartnerFuncionario(b.ctx, null, username, 'senha-segura-123'))
...
describe('Login — isolamento entre unidades', () => {
  it('credencial da unidade A não loga na unidade B', async () => {
    ...
    const a = await createPartnerFixture(db.pool);
    const b = await createPartnerFixture(db.pool);
    await q.createPartnerFuncionario(a.ctx, null, 'soa', 'senha-segura-123');
    // mesmo usuário+senha, mas no slug da unidade B → não loga
    expect(await q.authenticatePartnerLogin('test', b.slug, 'soa', 'senha-segura-123')).toBeNull();
  });
});
...
describe('Login — set-credentials sem overwrite', () => {
  it('login que já tem senha rejeita set-credentials por sessão (allowOverwrite=false)', async () => {
    const q = await importQueries();
    const f = await createPartnerFixture(db.pool);
    await q.setOwnPartnerCredentials(f.ctx, 'dono2', 'senha-segura-123', true); // 1º acesso
    await expect(q.setOwnPartnerCredentials(f.ctx, 'dono2b', 'outra-senha-456', false))
...
describe('Login — logout', () => {
  it('revokePartnerSession invalida a sessão atual', async () => {
    ...
    const auth = await importAuth();
    const f = await createPartnerFixture(db.pool);
    await q.createPartnerFuncionario(f.ctx, null, 'sai', 'senha-segura-123');
    const login = await q.authenticatePartnerLogin('test', f.slug, 'sai', 'senha-segura-123');
    expect(await auth.authenticatePartnerSession(f.slug, login!.session_token)).not.toBeNull();
