#!/usr/bin/env node
/**
 * PROVA DE PARIDADE — painel da MATRIZ (obra 300 de 2026-07-05; molde da prova
 * do parceiro, obra PLANO_REFATORACAO_PAINEL_300_2026-06-10.md).
 *
 * O QUE PROVA: a "impressao digital" da interface do painelApp() — o nome e o
 ...
 *
 * COMO: mocka o browser, carrega os app*.js NA ORDEM DO index.html dentro de um
 * vm isolado, chama painelApp() e extrai getOwnPropertyDescriptors (getters NAO
 * sao executados — reatividade intacta).
 *
 * USO:
 *   node scripts/prova-paridade-matriz.cjs                    -> compara com o baseline (exit 1 se diferente)
 *   node scripts/prova-paridade-matriz.cjs --gravar-baseline  -> (re)grava scripts/baseline-paridade-matriz.json
 *   node scripts/prova-paridade-matriz.cjs --dir <pasta>      -> usa outra pasta public (auto-teste/sabotagem)
 */
...
const BASELINE_PATH = path.join(__dirname, 'baseline-paridade-matriz.json');
...
function args() {
  ...
  return {
    ...
    dir: dirIdx >= 0 ? path.resolve(a[dirIdx + 1]) : path.join(RAIZ, 'painel', 'public'),
  };
}

/** Ordem REAL de carga: os <script src="/admin/painel/app*.js"> do index.html, na ordem. */
function listarScriptsDoIndex(publicDir) {
  ...
  if (arquivos.length === 0) {
    throw new Error(`nenhum <script src="/admin/painel/app*.js"> achado em ${indexPath} — regex quebrou ou HTML mudou`);
  }
  return arquivos;
}

/** Browser de mentira: o minimo pro top-level + corpo do painelApp() avaliarem. */
function criarSandbox() {
  ...
  const sandbox = {
    ...
    location: {
      ...
      href: 'http://localhost:4210/admin/painel',
      origin: 'http://localhost:4210', search: '', hash: '', reload() {},
    },
    localStorage: storage,
    sessionStorage: { getItem: () => null, setItem() {}, removeItem() {} },
    navigator: { userAgent: 'prova-paridade-matriz', language: 'pt-BR', clipboard: { writeText: () => Promise.resolve() } },
    document: {
...
function gerarManifesto(publicDir) {
  ...
  const app = vm.runInContext('painelApp()', ctx);
  if (!app || typeof app !== 'object') throw new Error('painelApp() nao devolveu objeto');
...
function main() {
  ...
  console.error(`\n[FALHA] PARIDADE QUEBROU: ${sumiram.length} sumiram, ${surgiram.length} apareceram, ${mudaram.length} mudaram.`);
  process.exit(1);
}
